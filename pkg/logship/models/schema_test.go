...
func TestTableResponse_ToLogshipTimeSeries(t *testing.T) {
	tests := []struct {
		name                  string
	...
	}{
		{
			...
			testFile:              "logship_timeseries_multi_label_multi_value.json",
		...
		},
		{
			...
			testFile:              "logship_timeseries_null_value_column.json",
			seriesCountIs:         require.Equal,
			seriesCount:           8,
			perSeriesValueCountIs: require.Equal,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			convertedFrame, err := ToLogshipTimeSeries(initialFrames[0])
			require.NoError(t, err)
